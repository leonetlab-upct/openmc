// traffic_generator.c - UDP traffic generator with run_id/sequence header and absolute timing
// Cambios: envía exactamente un múltiplo de 8 (floor(pps*duración/8)*8) para no perder el último bloque.
// Uso:
//   ./traffic-generator -a 10.102.96.2 -p 12345 -s 1000 -r 200 -t 10
...
#define DEFAULT_PPS  100       // datagrams por segundo
...
int main(int argc, char **argv) {
    ...
    printf("[traffic-generator] Sending UDP traffic to %s:%d | %d B | %d pps | %d s | run_id=0x%08x\n",
           ip_destino, puerto, tam_payload, pps, duracion, run_id);
    printf("[traffic-generator] Target adjusted to a multiple of 8: %u datagrams\n", target);
    ...
    printf("[traffic-generator] Finished. run_id=0x%08x, datagrams sent=%u (target=%u)\n",
           run_id, sent, target);
    free(buffer);
    close(sock);
    return 0;
