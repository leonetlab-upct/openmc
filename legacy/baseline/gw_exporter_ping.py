...
DEFAULT_IP_A      = "10.102.99.1"   # GS1
DEFAULT_IP_B      = "10.102.100.1"  # GS2
...
def metrics_worker(ip_a, ip_b, iface_a, iface_b,
    ...
    while True:
        ...
        # Escribir fichero para its_gateway_rq_istsrc.c, si tenemos valores
...
def main():
    parser = argparse.ArgumentParser(
        description="Exporter Python minimal para its_gateway_rq_istsrc:"
                    " mide ping ICMP a dos IP (GS1/GS2), calcula pérdida y RTT, "
                    "expone /metrics y escribe /tmp/its_metrics.txt"
    )
    parser.add_argument("--ip-a", default=DEFAULT_IP_A,
                        help=f"IP de GS1 / enlace A (por defecto: {DEFAULT_IP_A})")
    parser.add_argument("--ip-b", default=DEFAULT_IP_B,
                        help=f"IP de GS2 / enlace B (por defecto: {DEFAULT_IP_B})")
    ...
    parser.add_argument("--metrics-file", default=DEFAULT_METRICS_FILE,
                        help=f"Ruta del fichero para its_gateway (por defecto: {DEFAULT_METRICS_FILE})")
