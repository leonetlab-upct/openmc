/* ground_station_rq_istsrc.c
 *
 * Ground station for IST-SRC with RaptorQ (lcrq), equivalent in behavior
 * to ground_station_rs.c but replacing Reed–Solomon with RaptorQ.
 *
 ...
 *        immediately to servidor.c via raw IP on interface "term2term4",
 ...
 *
 * Build (ejemplo):
 *   gcc -O2 -Wall -o ground_station_rq_istsrc ground_station_rq_istsrc.c -llcrq
 */
...
int main(int argc, char *argv[])
{
    ...
    int udp_a = bind_udp_socket(IN_IFACE_A, 5000);
    int udp_b = bind_udp_socket(IN_IFACE_B, 5000);
    raw_sock  = create_raw_socket(RAW_OUT_IFACE);

    fprintf(stderr,
            "[GS] Listening on %s & %s, forwarding via %s\n",
            IN_IFACE_A, IN_IFACE_B, RAW_OUT_IFACE);
